...
def create_employee_draft(db: Session, payload: dict[str, Any]) -> tuple[Employee, dict[str, Any]]:
    ...
    employee = Employee(
        ...
        emergency_code=payload.get("emergency_code") or (payload.get("emergency_contact", {}).get("code") if isinstance(payload.get("emergency_contact"), dict) else None),
        bank_account_number=payload.get("bank_account_number") or None,
        ifsc_code=payload.get("ifsc_code") or None,
        bank_branch=payload.get("bank_branch") or payload.get("branch") or None,
