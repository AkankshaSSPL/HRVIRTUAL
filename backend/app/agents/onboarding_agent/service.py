...
class OnboardingAgent(BaseAgent):
    ...
    def execute(
        ...
        *,
        ...
        if not _is_new_onboarding_command(command):
            employee_id = active_entity_id or _latest_onboarding_finishing_employee_id(self.db, user_id)
        ...
        return self._turn(employee=employee, command=command, workflow_id=workflow_id, just_captured=just_captured, history=history)
    ...
    def _turn(self, *, employee: Employee, command: str, workflow_id: str, just_captured: dict[str, Any], history: list[dict[str, Any]] | None = None) -> dict[str, Any]:
        progress = compute_onboarding_progress(self.db, employee)
    ...
    def _reply(self, *, name: str, percent: int, section_label: str, ask_for: list[str], just_captured: dict[str, Any], completed: bool, history: list[dict[str, Any]] | None = None) -> str:
        if llm_available():
            try:
